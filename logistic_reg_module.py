...
#create the special class that we're going to use from here on to predict new data
class absenteeism_model():
    ...
    def load_and_clean_data(self, data_file):
        ...
        #to preserve the code we've created in the previous section, we'll add a column with 'NAN' strings
        df['Absenteeism in Hours'] = 'NaN'
        
        #create a separate dataframe, containing dummy values for All available reasons
        reason_columns = pd.get_dummies(df['Reason for Absence'], drop_first=True)
        
        #split the reason columns into 4 types
        ...
        #concatenate df and the 4 reason type columns
        df = pd.concat([df, reason_type_1, reason_type_2, reason_type_3, reason_type_4], axis=1)
        
        #assing names to the 4 reason type columns
        #NB: there is a more universal version of this code, 
        ...
        df['Day value'] = df['Date'].apply(lambda x: x.weekday())
        
        #drop the 'Date column from df'
        df = df.drop(['Date'], axis=1)
        
        #reorder the columns in df
        column_names_upd = ['Reason_1', 'Reason_2', 'Reason_3', 'Reason_4', 'Month value', 'Day value',
                            'Transportation Expense', 'Distance to Work', 'Age','Daily Work Load Average', 
                            'Body Mass Index', 'Education', 'Children', 'Pets', 'Absenteeism Time in Hours']
    ...
    def predicted_output_category(self):
        ...
        #predict the ouptuts and the probabilities and
